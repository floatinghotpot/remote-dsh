...
完整细节（含重启免配、多机同 token、吊销语义）见 [获取 join token](01-04-join-token.md)。三台机器各来一遍，列表里就有三台（含实时在线状态）。
...
- **改密**：portal 自助改密（验证当前密码）→ **全部已登录设备立即掉线**；忘记密码可**绑定邮箱后自助找回**（见 [账号安全与团队共享](01-08-account-security.md)），或找 admin 重置
