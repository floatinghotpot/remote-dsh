...
VPN 连上后，你的设备**就像在局域网里一样**（拿到一个内网 IP、能访问内网资源）。这时访问家里那台 DSH 智能体，**完全复用局域网玩法**：`rdsh host setup lan` + `rdsh host serve`，配对码照用。
...
```bash
# 家里那台跑 DSH 的主机上
rdsh host setup lan           # 写 host.json（mode: lan，默认 0.0.0.0:8443）
rdsh host serve
...
- **实测程度**：`rdsh host serve` 局域网访问能力已由 M1 验收；VPN 客户端连通是通用网络操作，具体以你的 VPN 配置为准
