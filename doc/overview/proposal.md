...
### 4.5 hub 对外 API 雏形（层 1，M3 前文档定稿）
...
- **原型期（M1–M3）**：全 TypeScript/Node —— rdsh-hub 以 TS 实现并随 `rdsh` npm 包分发，单一语言、迭代最快；
...
| **M2 云服务器直连** | TLS/https + **密码认证** + IP 白名单 + 配置文件 + 服务化（systemd/launchd）（2026-08-23 新增） | 云服务器 `rdsh service install` 常驻；https 下输密码可完整操作 DSH；config 的 `allow_from` 限定来源 |
| **M3 公网 hub** | rdsh-hub（注册/登录/host 绑定/路由）+ `rdsh join`（出站隧道）+ rdsh-portal（原 M2 顺延） | 异地浏览器登录 hub → 选择 host → 完整操作 DSH；token 吊销即时生效 |
| **M4 多租户增强** | 邮箱验证、2FA、共享授权、审计、限流（原 M3 顺延） | 安全加固项逐条过验收 |
| **M5 移动端 App** | rdsh-app（Flutter + WebView 壳）（原 M4 顺延） | App 登录后可访问 host 的 DSH |
| **M6 上线准备** | 域名备案、隐私政策、部署文档、压测（原 M5 顺延） | 达到公开服务标准 |
| **M7 hub Go 化 + E2E 评估** | rdsh-hub Go 单二进制（go:embed 门户）+ conformance 测试；公共 SaaS 化时实现 E2E（协议位已预留）（原 M6 顺延） | 单二进制部署；TS↔Go 互操作通过；E2E 需求评审 |
| **M8 微信小程序** | rdsh-weapp（原生小程序，wss 直连 hub API）—— 原 M7 顺延 | 小程序登录后可访问 host 的 DSH 轻量操作 |
...
| 微信小程序合规成本（备案/审核/域名） | 中 | 小程序独立 **M7**（2026-08-23 拆分），先用 App + H5 验证价值；备案尽早启动 |
...
| Q6 | hub 分发 | **提供 Docker 镜像**（2026-08-22 用户确认）：原型期包装 npm 包、生产期包装 Go 单二进制；Docker 是包装而非唯一路径，主分发形态分别为 npm 包 / 单二进制 |
| Q7 | 移动端范围 | **纯 WebView 壳**（Flutter）+ 登录态原生存储；验收须含 WebView 交互验证（剪贴板/输入法/文件下载），剪贴板不可用时加最小原生桥；原生推送/文件管理后置 |
...
（无 —— Q1–Q10 已全部定案，见上表。后续新决策进入 `doc/feature/01-remote-access/` 的 Feature Pipeline。）

---

*关联：本文档为产品提案（讨论稿）。进入开发前需按 Feature Pipeline 产出 `doc/feature/01-remote-access/req.md` 并由用户批准。*
