...
| **M6 上线准备** | 隐私政策/部署文档/压测 + 资质（招行聚合签约、B2 证确认、短信签名/模板、域名） | ⏳ 未开始（资质前置部分已具备：对公户/B2 证/ICP 已核验；签约与文档待办） |
...
- **资质（S3 前置，并行推进；2026-08-26 核验，详见 `08-saas/req.md` §5）**：
  ...
  - ⏳ 待办：**招行聚合收款签约**（确认动态码 API/费率结算——阻塞 S3）；**B2 证覆盖 B25 信息服务业务确认**（经营性 ICP，自营订阅必需；EDI 不适用——自营订阅无平台化）；**阿里云短信签名+模板审核**（1–3 天量级——阻塞短信真发，不阻塞 S1 mock）
- **内容**：隐私政策/用户协议/数据删除（S1 后 S4 前）、部署文档、压测（含 300 MB 大流量压测，M1 遗留项）
- **验收**：达到公开服务标准

### SaaS 托管轨（S1–S4，与 M6 并行 ⏳，见 `doc/feature/08-saas/`）

- **S1 注册·试用·配额**：开放注册（邮箱 / +86 手机号双通道，SmsSender mock 先行）、试用 3 天 1 host、配额钩子消费（08-saas R1/R2/R3/R11）—— 仅依赖 M5，可立即开工
