...
> **状态**: M1–M5 已完成并发布；**08-saas S1/S2 已实现**（注册双通道/试用/配额/订阅状态机/支付 mock + portal 全套 + i18n + 落地页 + 法务文档），**S3/S4 部分落地**（微信/招行/阿里云验证码代码就绪，验证码已真实验签；支付凭证与 E2E 加密待推进）；**09 云端 DSH / 10 Token 转售** 讨论中（discussion 已建）；M8/M9 降级为品牌/营销渠道
...
| **M6 上线准备** | 隐私政策/部署文档/压测 + 资质（招行聚合签约、B2 证确认、短信签名/模板、域名） | ⏳ 进行中（数据删除✅、隐私政策/协议草稿✅、资质部分核验✅；部署文档/压测/支付凭证待） |
| **SaaS 托管轨 S1–S4** | 商业化托管 hub：注册/试用/配额/订阅/支付（`08-saas`，与 M6 并行） | **S1/S2 ✅ 已实现**；**S3/S4 ⏳ 部分**（微信/招行支付 + 阿里云验证码代码就绪，支付凭证 + E2E 加密待推进） |
| **M8 移动端 App** | rdsh-app（Flutter，Android/iOS）—— **品牌/营销渠道** | ⏳ 后置（浏览器已验证可用，按需启动；**优先级高于 M7**） |
...
- **资质（S3 前置，并行推进；2026-08-26 核验，详见 `08-saas/req.md` §5）**：
  - ✅ 已具备：公司主体 + 招行对公户；增值电信业务经营许可证（川 B2 证）；ICP 备案（现有域名已通过；若 SaaS 用新域名需重新备案）；阿里云企业认证（短信/验证码/邮件复用）
  - ⏳ 待办：**支付通道（S3）**——微信支付/招行 provider 代码已就绪，待商户号（微信）或招行签约 + API 字段核对；**B2 证覆盖 B25 信息服务业务确认**（经营性 ICP，自营订阅必需；EDI 不适用——自营订阅无平台化）；**阿里云短信签名+模板审核**（1–3 天量级——阻塞短信真发，不阻塞 S1 mock）
- **内容**：隐私政策/用户协议（**草稿已写 `doc/saas/{terms,privacy}.md`，待法务定稿**）、部署文档、压测（含 300 MB 大流量压测，M1 遗留项）；数据删除已实现（`DELETE /api/account`，R7）
- **验收**：达到公开服务标准

### SaaS 托管轨（S1–S4，与 M6 并行 ⏳，见 `doc/feature/08-saas/`）

- **S1 注册·试用·配额** ✅：开放注册（邮箱 / +86 手机号双通道，PIN/短信码）、试用 3 天 1 host、配额钩子（R1/R2/R3/R11）
- **S2 订阅状态机·计费** ✅：套餐/订单/订阅表、两维状态机（trial→grace→free）、到期降级 + 30 天数据清理、账号删除、PaymentProvider mock（R4/R6/R7）
- **S3 支付接入** ⏳ 部分：`PaymentProvider` 抽象 + **微信支付**（`billing/wechatpay.ts`，RSA 签名 + HMAC 回调验签 + AES-GCM 解密，单测覆盖）+ **招行**（`billing/cmb.ts`，SM2 验签）代码就绪；**上线待商户号/密钥 + 招行 API 字段核对**
- **S4 反滥用·安全·运营** ⏳ 部分：算术验证码 + 限流 + 短信防轰炸 + banned 管理 + 付费审计已落地；**阿里云验证码 2.0 已真实验签通过（hub.unicgames.com）**；运营看板待
- **并行 P0**：E2E 加密（层 2 帧加密位，公共 SaaS 必须）—— 待实现
- **底座**：TS hub 已实现首发；M7 Go 化延后（触发式，见 M7）
...
- **定位（2026-08-26）**：**非用户必需**（笔记本/手机/微信内浏览器访问 hub 已实测可用）—— 定位**品牌/营销渠道**（应用商店曝光、品牌认知），后置、按需启动；**优先级高于 M7 hub Go 化**（品牌/营销渠道 > 非用户功能的重构）
...
2. **SaaS 商业化（08-saas）**：**S1/S2 已实现**（注册/试用/订阅/portal/支付 mock + i18n + 落地页 + 法务文档 + 备案/站点页脚）；剩余：微信/招行支付凭证、E2E 加密、法务定稿
...
| 2026-08-26 | **里程碑优先级调整**：**M8 移动端 App 前置于 M7 hub Go 化**——品牌/营销渠道（用户触达）优先于非用户功能的重构（触发式）；编号不变 |
| 2026-08-26 | **M6 资质核验落档**：对公户/川 B2 证/ICP/阿里云企业认证已具备（见 `08-saas/req.md` §5）；剩余招行签约 + B25 覆盖确认 + 阿里云短信签名/模板（均不阻塞 S1） |
| 2026-08-27 | **08-saas S1/S2 实现完成**：注册双通道/试用/配额/订阅状态机/PaymentProvider mock/账号删除/30 天 host 清理/banned 管理；portal 全套（注册/验证/登录/订阅/设置 + i18n + 落地页 + 备案/站点页脚 + 法务文档构建期 md→html）；微信支付/招行/阿里云验证码三通道代码就绪（验证码已真实验签通过 hub.unicgames.com）；用户协议/隐私政策草稿落 `doc/saas/` 待法务定稿 |
