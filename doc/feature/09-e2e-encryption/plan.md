...
- [x] `hub/src/relay.ts` + `server.ts`：`/e2e/<hostId>` 路由（WS upgrade → raw 字节双向互转）；HTML `<head>` 注入 `E2EE_SHIM_HTML`（`mode !== "off"` 时）
...
> **实现说明（相对 solution 的偏离）**：数据面 E2EE 采用「hub 注入 shim 脚本包装 `window.fetch` + `window.WebSocket`」单页方案，替代原 SW + `ws-shim.js` 双文件方案（避免 SW scope/bundling/postMessage 复杂度，solution §6/§7 已记录决策）。
...
**待验证（需浏览器测试环境，用户执行）**：
- shim fetch/WS 包装在真实 DSH 页面生效、`/h/` 走 `/e2e` 密文通道
- pin 首次信任/指纹变更告警 UI 交互
- Noise 直实现跨实现互操作（本轮已用 Node↔WebCrypto 互操作测试覆盖；生产前建议再对照 Noise 规范/成熟库交叉核对，见 solution §7 watch-list）
