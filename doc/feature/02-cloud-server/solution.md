...
`rdsh service install` 后常驻；浏览器经 HTTPS 输 user/pass 登录（或 pair/none），复用 M1 的会话 Cookie 与转发内核。**三种部署用例**（req R1）：① 内置 TLS（用户证书）；② apache2 反代 + cron acme.sh 自动续期；③ nginx 反代 —— 部署配置见 `doc/overview/usage.md` §7，验收 = [req.md](req.md) §3 端到端场景。
...
| 依赖 | 仅 `ws` | 新增依赖：**无**（scrypt/TLS/CIDR 全内置） |
...
| 需求 | 现状 → 目标 |
|---|---|
| R1/R2 TLS | node:http → **node:https**（`tls.cert/key` 配置；无证书即 http） |
...
| `src/tls.ts`（新） | TLS 证书 | **任意 PEM**（`tls.cert/key` 路径，兼容 acme.sh/Let's Encrypt/云厂商/手动 openssl）；无配置或 `behindProxy` → 返回 null（http）；**不内置自签生成** —— 需要 https 由用户提供证书 |
...
- **新增 npm 依赖：无**（scrypt/https/CIDR 内置）
...
| `server.test.ts`（扩展） | password 模式登录流程/改密后旧 Cookie 307/allow_from 403/TLS 启动（一次性证书 + curl -k）/无 TLS+password 拒绝 |
...
| 项 | 定稿 |
|---|---|
| 证书来源 | **两种方式并存**：① 内置 TLS —— 用户提供任意 PEM（acme.sh/Let's Encrypt/云厂商/手动 openssl 自签）填 `tls.cert/key`；acme.sh 续期用 renew hook 调 `rdsh service restart`。② **反代 TLS** —— `behindProxy: true`，nginx/apache/certbot 管证书与 HTTPS，rdsh 监听本地 http（WebSocket 需反代 upgrade 头）。**无证书即 http**（仅 pair/none）；password 无证书非反代拒绝启动 |
...
| 风险 | 对策 |
|---|---|
| 证书由用户提供的门槛 | 日志引导（`auth.mode=password requires TLS` + 证书来源建议）；部署文档列 acme.sh/云厂商/手动 openssl 三种获取方式 |
