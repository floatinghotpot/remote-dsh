...
> **驱动文档**: [`what-and-why.md`](what-and-why.md) —— **是什么 / 为什么做 / 值多少 / 什么时候必须做**（**排期与取舍先看它**；本文只管"做过什么、事实是什么、决定了什么"）
...
**命名核查与占名（2026-09-11）**：npm 包名 `dsh-agent-mesh` **未注册、可用**（registry 返回 404 `{"error":"Not found"}` = 从未发布，非被撤下）。近邻提醒：第三方包 **`dsh-mesh`**（Wang-Lin-Chang，Apache-2.0）是**单机崩溃隔离的多进程编排库**（文件即消息 + `O_EXCL` 租约 + dead-letter 收养；轴不同，但撞词，README 需写"勿混淆"），**`dsh-agent`** 是他人（dushaobindoudou）占位。为防抢注，已在仓库建立**真包骨架** `packages/agent-mesh@0.0.1` 作为**占名版本**（无功能、调用即抛错、README 明确标注"仅占名"），P0 实现时递增版本续发。备选名（若该名不可用或需更强区隔）：`dsh-host-mesh`、`rdsh-agent-mesh`（`rdsh-*` 已是我方三个包）。
