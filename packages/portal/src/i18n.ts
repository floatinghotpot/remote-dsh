...
const EN: Record<string, string> = {
  ...
  "设置步骤：": "Setup steps:",
  "任选一款验证器 App 并打开（见上方清单）": "Open the authenticator app you picked (see the list above)",
  ...
  "（自己的常用设备可勾选；公用或他人的设备请勿勾选）": "(check this on your own device only — don't check it on shared or other people's devices)",
  "登录时可勾选「记住此设备 30 天」：自己的常用设备勾上，30 天内免输动态码；公用/他人设备请勿勾选；同一浏览器会话内也不会反复要求验证码": "At sign-in you can check “Trust this device for 30 days”: check it on your own device to skip the code for 30 days — don't check it on shared or others' devices. Within the same browser session, the code isn't asked repeatedly.",
  "若换机或丢失验证器，需联系平台支持重置 2FA": "If you switch phones or lose your authenticator, contact platform support to reset 2FA",
  "密钥（手动添加账号时粘贴）：": "Secret (paste it when adding the account manually):",
