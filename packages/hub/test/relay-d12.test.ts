...
test("D12 cookie 白名单：仅放行 rdsh_gate，剥离会话 cookie", () => {
  // 带 rdsh_gate + rdsh_hub_session + authorization → 只保留 rdsh_gate
  const out = normalizeHeaders({
    cookie: "rdsh_hub_session=abc; rdsh_host=xyz; rdsh_gate=gate1",
  ...
  assert.equal(out["rdsh_hub_session" as never], undefined);
...
test("D12：无 rdsh_gate 时剥离全部 cookie", () => {
  const out = normalizeHeaders({ cookie: "rdsh_hub_session=abc; rdsh_host=xyz" } as never);
  assert.equal(out["cookie"], undefined);
});

test("D12：cookie 数组形态", () => {
  const out = normalizeHeaders({ cookie: ["rdsh_hub_session=a", "rdsh_gate=g2"] } as never);
